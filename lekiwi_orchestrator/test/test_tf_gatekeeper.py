...
"""Unit tests for TfReadinessGatekeeper logic and parameters."""

import math
import pytest

from lekiwi_orchestrator.tf_gatekeeper import (
...
def test_robot_stationary_detection():
    # Inside standstill limits (speed = hypot(0.01, 0.01) = 0.0141 <= 0.03, wz = 0.02 <= 0.08)
    assert check_robot_stationary(
        0.01, 0.01, 0.02, max_linear_vel=0.03, max_angular_vel=0.08
    )

    # Exactly zero
    assert check_robot_stationary(0.0, 0.0, 0.0)

    # Exceeding linear velocity limit (0.05 m/s > 0.03 m/s)
    assert not check_robot_stationary(
        0.05, 0.0, 0.0, max_linear_vel=0.03, max_angular_vel=0.08
    )

    # Exceeding angular velocity limit (0.1 rad/s > 0.08 rad/s)
    assert not check_robot_stationary(
        0.005, 0.005, 0.1, max_linear_vel=0.03, max_angular_vel=0.08
    )
...
def test_covariance_convergence():
    ...
    cov = [0.0] * 36
    cov[0] = 0.0004  # var(x)
    cov[7] = 0.0004  # var(y) -> sum = 0.0008 <= 0.0012
    cov[35] = 0.0020  # var(yaw) <= 0.0030

    converged, pos_var, yaw_var = check_covariance_converged(
        cov, max_pos_var=0.0012, max_yaw_var=0.0030
    )
    assert converged
    assert math.isclose(pos_var, 0.0008)
    assert math.isclose(yaw_var, 0.0020)

    # High position variance (0.0015 > 0.0012)
    cov_high_pos = list(cov)
    cov_high_pos[0] = 0.0010
    cov_high_pos[7] = 0.0005  # sum = 0.0015
    converged, _, _ = check_covariance_converged(
        cov_high_pos, max_pos_var=0.0012, max_yaw_var=0.0030
    )
    assert not converged

    # High yaw variance (0.0050 > 0.0030)
    cov_high_yaw = list(cov)
    cov_high_yaw[35] = 0.0050
    converged, _, _ = check_covariance_converged(
        cov_high_yaw, max_pos_var=0.0012, max_yaw_var=0.0030
    )
    assert not converged
...
def test_stamp_freshness():
    # Fresh (age 0.05s <= 0.30s)
    is_fresh, age = check_stamp_freshness(
        current_time_sec=100.05, stamp_time_sec=100.0, max_age_sec=0.30
    )
    assert is_fresh
    assert math.isclose(age, 0.05)

    # Stale (age 0.35s > 0.30s)
    is_fresh, age = check_stamp_freshness(
        current_time_sec=100.35, stamp_time_sec=100.0, max_age_sec=0.30
    )
    assert not is_fresh
    assert math.isclose(age, 0.35)
