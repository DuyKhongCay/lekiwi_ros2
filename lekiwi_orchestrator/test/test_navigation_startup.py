...
def startup(monkeypatch):
    ...
    clock = [10.0]
    monkeypatch.setattr("lekiwi_orchestrator.navigation_startup.time.monotonic", lambda: clock[0])
    node = Mock()
    node.declare_parameter.side_effect = lambda name, default: SimpleNamespace(value=default)
...
def test_requires_live_readiness_and_starts_once(startup):
    ...
    helper.on_ready(Bool(data=True))
    helper.tick()
    assert client.call_async.call_args.args[0].command == ManageLifecycleNodes.Request.STARTUP
