/**
 * @file chessboard_postprocess.cpp
 * @brief Implementation of homography calculation and perspective transform for chessboard.
 *
 * @author DuyKhongCay
    ...
    static bool decode(HailoROIPtr roi, ChessboardCandidateResult &out_result)
    {
      ...
      for (size_t idx = 0; idx < tensors.size(); ++idx)
      {
        ...
        else
        {
          ...
          {
            if (c == 64)
              cv2_tensors[stride] = t;
            else if (c == 32)
              cv4_tensors[stride] = t;
            else if (c <= 80)
              cv3_tensors[stride] = t;
          }
        }
      }

      if (!proto_tensor || cv3_tensors.empty())
        return false;
...
#pragma GCC unroll 32
      ...
      std::vector<cv::Point2f> &out_inliers, float threshold, int max_iters)
  {
    ...
    for (int iter = 0; iter < max_iters; ++iter)
    {
      ...
      if (len_sq < (min_sample_dist * min_sample_dist))
        continue;
      ...
      float threshold, int max_iters)
  {
    ...
    if (!lineIntersection(lines[3], lines[0], c0))
      c0 = init_corners[0];
    if (!lineIntersection(lines[0], lines[1], c1))
      c1 = init_corners[1];
    if (!lineIntersection(lines[1], lines[2], c2))
      c2 = init_corners[2];
