...
"""
Adapter Pattern: Bridge LeRobot Arm Messages to FollowJointTrajectory.

Adapts between HuggingFace LeRobot's raw/degrees/radians joint representation
and the standard ROS 2 ros2_control FollowJointTrajectory action interface.
"""
...
def load_joint_config(joint_config_file: str) -> Dict[str, JointCalibration]:
    ...
    joint_config_file : str
        Path to calibration YAML file (e.g. sts3215_servos_calib.yaml).
...
class LeRobotArmBridge(Node):
    """
    Adapter component translating named LeRobot arm actions into ROS FollowJointTrajectory goals,
    and republishing arm joint states in LeRobot calibration units.
    """

    def __init__(self, node_name: str = "lerobot_arm_bridge"):
        ...
        joint_config_file = str(self.get_parameter("joint_config_file").value)
        self._calibration = (
        ...
        self._units = str(self.get_parameter("action_units").value).lower()
        ...
        self._observation_pub = self.create_publisher(
            JointState, str(self.get_parameter("observation_topic").value), action_qos
        )
        ...
        self.get_logger().info(
            f"LeRobotArmBridge initialized (units={self._units}, trajectory_dur={duration}s)."
        )

    def _on_action(self, message: JointState) -> None:
        ...
        try:
            ...
            if not goal_handle.accepted:
                self.get_logger().error(
                    "arm_controller rejected LeRobot trajectory goal"
                )
    ...
    def _named_values(self, message: JointState) -> Optional[Dict[str, float]]:
        ...
        if missing:
            self.get_logger().warning(
                f"Ignoring partial arm message; missing {missing}",
                throttle_duration_sec=2.0,
            )
            return None
        return values
