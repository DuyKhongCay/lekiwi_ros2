...
| **Hand-Eye Calibration Extrinsics** (`handeye_calib_stereo_left.yaml`) | Calibrated 6-DoF spatial transform from `base_footprint` to `stereo_left_optical` | Loaded via xacro macro `sensors_calibration.xacro` in `lekiwi_robot.urdf.xacro` | Mismatched extrinsics causing vision pick-and-place offset errors | [`lekiwi_description/calib/handeye_calib_stereo_left.yaml`](file:///root/docker_ws/lekiwi_ros2/lekiwi_description/calib/handeye_calib_stereo_left.yaml) |
